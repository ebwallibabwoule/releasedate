...
echo '<html><head><title>getID3() - Sample tag writer</title></head><style type="text/css">BODY,TD,TH { font-family: sans-serif; font-size: 9pt;" }</style><body>';
	...
	if (file_exists($Filename)) {
		...
		echo '<tr><td align="right"><b>Arrstigty</b></td> <td><input type="text" size="40" name="Artist"  value="'.htmlentities((!empty($OldThisFileInfo['comments']['artist'])  ? implode(', ', $OldThisFileInfo['comments']['artist'] ) : ''), ENT_QUOTES).'"></td></tr>';
...
echo '</body></html>';
...
else if (!empty($Filename) && !empty($format)) {
	...
	echo '{"tag": {"artist": "' .
	 	 htmlentities((!empty($OldThisFileInfo['comments']['artist']) ? implode(', ', $OldThisFileInfo['comments']['artist']) : ''), ENT_QUOTES)
		 . '", "title": "' .
	     htmlentities((!empty($OldThisFileInfo['comments']['title'])  ? implode(', ', $OldThisFileInfo['comments']['title'] ) : ''), ENT_QUOTES)
		 . '", "year": "' .
